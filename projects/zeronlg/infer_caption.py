...
if __name__ == '__main__':
    ...
    parser.add_argument('--mean_pooling_dim', type=int, default=1, choices=[0, 1])
    ...
    for mode in args.modes:
        ...
        pickle_path = get_formatted_string(vars(args), 'pickle_path', assigned_kwargs=dict(
            dataset=args.dataset, clip_model_name=model.clip_model_name.replace('/', '_'), mode=mode, postfix='_tokens' if model.use_clip_tokens else ''
        ))
        ...
        dataset = CaptionDataset(
            ...
            mean_pooling_dim=args.mean_pooling_dim,
        )
