...
def download_if_necessary(
        ...
        key_files: str = ['config.json', 'pytorch_model.bin', 'model.safetensors', 'pytorch_model.bin.index.json'],
    ...
    key_model_files = [f for f in key_files if 'model' in f]
    key_files = [f for f in key_files if 'model' not in f]
    
    need_to_download_flag = False
    if len(key_model_files) and not any(os.path.exists(os.path.join(model_path, f)) for f in key_model_files):
        need_to_download_flag = True
    if any(not os.path.exists(os.path.join(model_path, f)) for f in key_files):
        need_to_download_flag = True
