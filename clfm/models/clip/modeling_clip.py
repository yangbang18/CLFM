...
class CLIPAttention(nn.Module):
    ...
    def forward(
        ...
        if prompt is not None:
            ...
            prompt_key = self._shape(prompt_key, -1, bsz)
            prompt_value = self._shape(prompt_value, -1, bsz)
            
            key_states = torch.cat((prompt_key, key_states), dim=2)
            value_states = torch.cat((prompt_value, value_states), dim=2)

            prefix_mask = attention_mask.new_zeros(bsz, 1, tgt_len, prompt_key.size(2))
            attention_mask = torch.cat((attention_mask, prefix_mask), dim=3)
...
class CLIPEncoder(nn.Module):
    ...
    def forward(
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            # Added by Yang B.
            prompt_of_this_layer = None
            if prompt is not None:
                if isinstance(prompt[0], (list, tuple)):
                    if idx < len(prompt[0]):
                        prompt_of_this_layer = (prompt[0][idx], prompt[1][idx])
                        if prompt[0][idx] is None or prompt[1][idx] is None:
                            assert prompt[0][idx] is None
                            assert prompt[1][idx] is None
                            prompt_of_this_layer = None
                elif prompt[0].ndim == 4 and idx < prompt[0].size(0):
                    prompt_of_this_layer = (prompt[0][idx], prompt[1][idx])
                elif prompt[0].ndim == 3 and idx == 0:
                    prompt_of_this_layer = prompt
            ...
            if self.gradient_checkpointing and self.training:
                layer_outputs = self._gradient_checkpointing_func(
                    ...
                    output_attentions,
                    prompt_of_this_layer, # Added by Yang B.
                )
            else:
                layer_outputs = encoder_layer(
                    ...
                    causal_attention_mask,
                    output_attentions=output_attentions,
                    prompt=prompt_of_this_layer, # Added by Yang B.
                )
...
class CLIPAttentionWithAdapters(CLIPAttentionAdaptersMixin, CLIPAttention):
    def forward(
        ...
        # apply the causal_attention_mask first
        if causal_attention_mask is not None:
            # Fix bug: torch.ones --> torch.zeros (Yang B.)
            prefix_mask = torch.zeros(
                bsz, 1, causal_attention_mask.size(2), src_len - causal_attention_mask.size(-1)
            ).to(causal_attention_mask.device)
            causal_attention_mask = torch.cat([prefix_mask, causal_attention_mask], dim=-1)
